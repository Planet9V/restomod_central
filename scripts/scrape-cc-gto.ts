/**
 * Scrape ClassicCars.com Pontiac GTOs
 *
 * Scrapes Pontiac GTO listings from ClassicCars.com using Playwright
 *
 * Usage:
 *   tsx scripts/scrape-cc-gto.ts
 */
...
async function scrapeGTOs(targetCount: number = 40, pages: number = 3) {
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('🚗  Scraping ClassicCars.com Pontiac GTOs');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
  ...
  try {
    for (let pageNum = 1; pageNum <= pages; pageNum++) {
      ...
      if (pageNum === 1) {
        await page.screenshot({ path: 'debug-gto.png', fullPage: false });
        console.log('   📸 Screenshot saved to debug-gto.png');
      }
      ...
      const vehicles = await page.evaluate(() => {
        ...
        cards.forEach((card) => {
          try {
            ...
            const modelMatch = titleText.match(/GTO\s+(.+?)(?:\s|$)/i);
            if (modelMatch) {
              model = `GTO ${modelMatch[1].trim()}`;
            } else if (titleText.toLowerCase().includes('gto')) {
              model = 'GTO';
            }
...
async function main() {
  try {
    const vehicles = await scrapeGTOs(40, 3);

    // Save to file
    const outputPath = join(process.cwd(), 'data', 'scraped-cc-gto.json');
