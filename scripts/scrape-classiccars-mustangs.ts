...
async function scrapeClassicCarsMustangs() {
  console.log('🚀 Starting ClassicCars.com Mustang scraper...\n');
  ...
  const baseUrl = 'https://classiccars.com/listings/find?make=ford&model=mustang';
      ...
      const cars = await page.evaluate(() => {
        ...
        carElements.forEach((element) => {
          try {
            ...
            // Parse title like "1967 Ford Mustang"
            const titleMatch = titleText.match(/(\d{4})\s+([\w\s]+?)\s+([\w\s]+?)(?:\s|$)/);
            const year = titleMatch ? parseInt(titleMatch[1]) : 0;
            const make = titleMatch ? titleMatch[2].trim() : 'Ford';
            const model = titleMatch ? titleMatch[3].trim() : 'Mustang';
...
// Run the scraper
scrapeClassicCarsMustangs()
  .then(cars => {
    console.log(`✨ Successfully scraped ${cars.length} Mustangs!`);
    process.exit(0);
  })
  .catch(error => {
