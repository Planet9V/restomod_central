...
CREATE TABLE `builder_profiles` (
	...
	`certifications` text,
	`warranty_offered` integer DEFAULT 0,
...
CREATE TABLE `car_show_events` (
	...
	`awards` text,
	`parking_info` text,
	`food_vendors` integer DEFAULT 0,
	`swap_meet` integer DEFAULT 0,
	`live_music` integer DEFAULT 0,
	`kids_activities` integer DEFAULT 0,
...
CREATE UNIQUE INDEX `car_show_events_event_slug_unique` ON `car_show_events` (`event_slug`);
...
CREATE UNIQUE INDEX `cars_for_sale_stock_number_unique` ON `cars_for_sale` (`stock_number`);
...
CREATE UNIQUE INDEX `configurator_bodywork_options_bodywork_id_unique` ON `configurator_bodywork_options` (`bodywork_id`);
...
CREATE UNIQUE INDEX `configurator_customer_configurations_configuration_id_unique` ON `configurator_customer_configurations` (`configuration_id`);
...
CREATE UNIQUE INDEX `configurator_fuel_system_options_fuel_system_id_unique` ON `configurator_fuel_system_options` (`fuel_system_id`);
...
CREATE UNIQUE INDEX `configurator_glass_options_glass_id_unique` ON `configurator_glass_options` (`glass_id`);
...
CREATE UNIQUE INDEX `configurator_rear_axle_options_axle_id_unique` ON `configurator_rear_axle_options` (`axle_id`);
...
CREATE UNIQUE INDEX `configurator_suspension_options_suspension_id_unique` ON `configurator_suspension_options` (`suspension_id`);
...
CREATE UNIQUE INDEX `enhanced_engine_options_engine_id_unique` ON `enhanced_engine_options` (`engine_id`);
...
CREATE UNIQUE INDEX `enhanced_interior_options_interior_id_unique` ON `enhanced_interior_options` (`interior_id`);
...
CREATE UNIQUE INDEX `enhanced_transmission_options_transmission_id_unique` ON `enhanced_transmission_options` (`transmission_id`);
...
CREATE UNIQUE INDEX `enhanced_vehicle_platforms_platform_id_unique` ON `enhanced_vehicle_platforms` (`platform_id`);
...
CREATE TABLE `gateway_vehicles` (
	...
	`power_brakes` integer DEFAULT 0,
	`power_windows` integer DEFAULT 0,
	`power_seats` integer DEFAULT 0,
	`heated_seats` integer DEFAULT 0,
	`leather_seats` integer DEFAULT 0,
	`sunroof` integer DEFAULT 0,
	`convertible` integer DEFAULT 0,
	`hardtop` integer DEFAULT 0,
	`tilt_wheel` integer DEFAULT 0,
	`cruise_control` integer DEFAULT 0,
	`am_fm_radio` integer DEFAULT 0,
	`cd_player` integer DEFAULT 0,
	`cassette` integer DEFAULT 0,
	`premium_sound` integer DEFAULT 0,
	`alarm_security` integer DEFAULT 0,
	`keyless_entry` integer DEFAULT 0,
	`anti_lock_brakes` integer DEFAULT 0,
	`driver_airbag` integer DEFAULT 0,
	`passenger_airbag` integer DEFAULT 0,
	`side_airbags` integer DEFAULT 0,
	`traction_control` integer DEFAULT 0,
	`stability_control` integer DEFAULT 0,
...
CREATE UNIQUE INDEX `gateway_vehicles_stock_number_unique` ON `gateway_vehicles` (`stock_number`);
...
CREATE TABLE `investment_analytics` (
	...
	`last_analyzed` integer DEFAULT (cast((julianday('now') - 2440587.5)*86400000 as integer)),
	`created_at` integer DEFAULT (cast((julianday('now') - 2440587.5)*86400000 as integer))
);
...
CREATE UNIQUE INDEX `luxury_showcases_slug_unique` ON `luxury_showcases` (`slug`);
...
CREATE TABLE `research_articles` (
	...
	`publish_date` integer DEFAULT (cast((julianday('now') - 2440587.5)*86400000 as integer)) NOT NULL,
...
CREATE UNIQUE INDEX `research_articles_slug_unique` ON `research_articles` (`slug`);
