...
async function scrapeTempestLeMans() {
  ...
  const allCars = [];
  const baseUrl = 'https://classiccars.com/listings/find?year-min=1964&year-max=1972&make=pontiac';

  // Scrape multiple pages until we have 40 Tempest/LeMans cars
  for (let pageNum = 1; pageNum <= 6; pageNum++) {
    ...
    const cars = await page.evaluate(() => {
      ...
      listingElements.forEach((listing) => {
        try {
          ...
          const yearMatch = yearText.match(/\b(19[6-7]\d)\b/);
          ...
          // Parse title like "1967 Pontiac Tempest" or "1968 Pontiac LeMans"
          ...
          // Only include Tempest, LeMans, and related models (excluding pure GTOs)
          const modelLower = model.toLowerCase();
          const isTempestLeMans =
            modelLower.includes('tempest') ||
            modelLower.includes('lemans') ||
            modelLower.includes('le mans') ||
            (modelLower.includes('gto') && (modelLower.includes('lemans') || modelLower.includes('tempest')));

          if (!isTempestLeMans) {
            return; // Skip non-Tempest/LeMans models
          }
    ...
    console.log(`Found ${cars.length} Tempest/LeMans cars on page ${pageNum}`);
    allCars.push(...cars);
  ...
  console.log(`\nTotal Tempest/LeMans cars scraped: ${allCars.length}`);
